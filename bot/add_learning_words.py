...
def get_learning_words(update: telegram.Update, context: telegram.ext.CallbackContext):
    ...
    check_number_of_words(update=update, context=context, learning_words=learning_words)
...
def check_number_of_words(update: telegram.Update, context: telegram.ext.CallbackContext, learning_words):
    ...
    if len(learning_words) == configurations.settings.NUMBER_OF_WORDS:  # Проверяет кол-во введённых слов,
        ...
        accepted_words = accepted_words_visual(learning_words=learning_words,
                                               learning_words_translated=translate_learnings_words(
                                                   update=update,
                                                   context=context,
        ...
        words_not_accepted(update=update, context=context, learning_words=learning_words)
...
def translate_learnings_words(update: telegram.Update, context: telegram.ext.CallbackContext, learning_words):
    ...
    for word in range(len(learning_words)):  # Добавляет в learning_words_translated переведённые слова.
        learning_words_translated.append(translators.google(query_text=learning_words[word],
...
def words_not_accepted(update: telegram.Update, context: telegram.ext.CallbackContext, learning_words):
