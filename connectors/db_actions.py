...
class DataBase:
    ...
    async def find_user_id_record(self, message: aiogram.types.Message) -> bool:
        with self.__conn:
            self.__cur.execute(f'SELECT user_id FROM user_data WHERE user_id == "{message.from_user.id}"')
        return bool(self.__cur.fetchone())

    async def create_user_record(self, message: aiogram.types.Message):
        with self.__conn:
            self.__cur.execute(f'INSERT INTO user_data (user_id) VALUES ({message.from_user.id})')

    async def add_learned_words(self, learned_words: List[str], message: aiogram.types.Message):
        with self.__conn:
            if not await self.find_user_id_record(message=message):
                await self.create_user_record(message=message)
    ...
    async def update_best_score(self, score: int, message: aiogram.types.Message):
        with self.__conn:
            if not await self.find_user_id_record(message=message):
                await self.create_user_record(message=message)
            self.__cur.execute(
    ...
    async def get_best_score(self, message: aiogram.types.Message) -> int:
        with self.__conn:
            if not await self.find_user_id_record(message=message):
                await self.create_user_record(message=message)
